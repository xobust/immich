    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MergePersonDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MergePersonDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
